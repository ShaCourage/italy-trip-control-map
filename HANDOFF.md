...
- 저장 상태 방어: `src/lib/tripDays.ts`, `src/lib/customPlaces.ts`가 localStorage/백업 데이터를 정규화해
  날짜의 `route/fallback/checklist` 누락, 사용자 장소의 배열 필드 누락으로 화면이 깨지는 회귀를 차단.
