...
- 저장 상태 방어: `src/lib/tripDays.ts`, `src/lib/customPlaces.ts`, `src/lib/appSettings.ts`가
  localStorage/백업 데이터를 정규화해 날짜/사용자 장소/시작 탭·필터 설정 손상으로 화면이 깨지는 회귀를 차단.
