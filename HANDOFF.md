...
- 동작 확인됨: 빈 셋업 → 템플릿 4종 적용, 루트 편집(잠금/숙소 복귀 규칙), 백업(v6)·복원,
  문서함, 저장소 v3 마이그레이션, 콘솔 에러 0
...
### 4. B4 — 공식 영업시간 링크 고정 (`DESIGN.md` §7 P1)
"지금 영업 중" 추정은 객관성 원칙(P1)상 금지. 대신 장소 카드에 공식 영업시간/예약 페이지 링크를 고정 노출.
(추정 라벨 만들지 말 것.)

### 5. P2 — 여유 있으면 (`DESIGN.md` §7)
- **C1** 예산 기록 — 하루 지출 메모 수준(가계부 아님)
- **C2** 일몰 시간 — 일부 완료(오늘 화면 계산값). 해질녘 추천 모드와 더 연동 가능
